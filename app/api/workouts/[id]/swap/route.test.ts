...
describe('Workout Swap API', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    (headers as any).mockResolvedValue(new Headers());
  });

  it('POST swaps exercise successfully', async () => {
    (auth.api.getSession as any).mockResolvedValue({ user: { id: 'u1' } });
    (services.runEffect as any).mockResolvedValue({});

    const req = new NextRequest('http://localhost/api/workouts/w1/swap', {
      method: 'POST',
      body: JSON.stringify({ exerciseOrder: 0, newExerciseId: 'e2', newExerciseName: 'New' }),
    });
